...
module.exports = {
  ...
  url: function ({ channel, date }) {
    return `https://www.tvhebdo.com/horaire-tele/${
      channel.site_id
  ...
  async channels() {
    ...
    return items.map((item) => {
      const $item = cheerio.load(item);
      const name = $item(
        ".gr_row_head > div > a.gr_row_head_logo.link_to_station > img"
      ).attr("alt");
      const url = $item(".gr_row_head > div > div.gr_row_head_poste > a").attr(
        "href"
      );
